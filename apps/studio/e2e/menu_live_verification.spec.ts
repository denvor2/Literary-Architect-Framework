...
test.describe('Menu System Live Verification', () => {
  ...
  test('File: Новая книга creates book', async ({ page }) => {
    ...
    const booksBefore = await page.getByRole('button').filter({ hasText: /Книга|Book/ }).count();

    await page.getByLabel(/Новая книга|New Book/i).click();

    const booksAfter = await page.getByRole('button').filter({ hasText: /Книга|Book|Без названия/ }).count();
  ...
  test('View: Theme toggle buttons visible', async ({ page }) => {
    ...
    await expect(page.getByRole('button').filter({ hasText: /Светлая|Light/ })).toBeVisible();
    await expect(page.getByRole('button').filter({ hasText: /Тёмная|Dark/ })).toBeVisible();
    await expect(page.getByRole('button').filter({ hasText: /Авто|Auto/ })).toBeVisible();
  });

  test('View: Sidebar toggle visible', async ({ page }) => {
    await page.getByLabel(/Меню Вид|View/i).click();
    const sidebarToggle = page.getByRole('button').filter({ hasText: /Боковая панель|Sidebar/ }).first();
  ...
  test('Help: Горячие клавиши button visible', async ({ page }) => {
    ...
    const shortcutsBtn = page.getByRole('button').filter({ hasText: /Горячие клавиши|Shortcuts/ }).first();
    await expect(shortcutsBtn).toBeVisible();
  });

  test('Help: Документация link visible', async ({ page }) => {
    await page.getByLabel(/Меню Руководство|Help/i).click();
    const docsBtn = page.getByRole('button').filter({ hasText: /Документация|Documentation/ }).first();
  ...
  test('Keyboard: Escape closes menu', async ({ page }) => {
    ...
    // Menu should be open
    const menuDropdown = page.locator('nav').locator('div').filter({ has: page.getByLabel(/Новая книга|New Book/i) }).first();
