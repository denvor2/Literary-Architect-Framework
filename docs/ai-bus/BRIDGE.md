# AI Bus — Sprint Bridge

## Purpose

Resolve the dual source-of-truth risk between the Sprint planning layer
([CURRENT_SPRINT.md](../project/CURRENT_SPRINT.md)) and the AI Bus execution layer
...
- **Sprint system** (`docs/project/CURRENT_SPRINT.md`) is the **planning authority**. It
  ...
  done. It is never modified by AI Bus activity.
- **AI Bus** (`docs/ai-bus/`, `docs/project/CURRENT_STEP.md`) is the **execution layer**. It
...
- Every Sprint step (`Sprint-<NN>-Step-<X>`, e.g. `Sprint-04-Step-02`) may be executed through
  one or more AI Bus steps, numbered `Sprint-<NN>-Step-<X>.<Y>` — e.g. `Sprint-04-Step-02.1`,
  `Sprint-04-Step-02.2`.
...
- A Sprint step in `CURRENT_SPRINT.md` is only marked `[x]` (done) once its highest-numbered
  mapped AI Bus step has received `STATUS: OK` under [REVIEW_FORMAT.md](REVIEW_FORMAT.md).
- `CURRENT_STEP.md`'s `id` MUST always be a valid `X.Y` under this scheme, resolving back to
  exactly one Sprint step `X`. An AI Bus step with no corresponding Sprint step is invalid
  under this bridge.

## Authority Rules

- `CURRENT_SPRINT.md` remains the sole planning authority: it defines the sprint's steps,
  their scope, and their order. AI Bus does not add, remove, or reorder Sprint steps.
- `CURRENT_STEP.md` is a **runtime pointer only** — it names which AI Bus step is executing
  right now and which Sprint step it belongs to. It carries no independent scope-defining
  authority.
- AI Bus never replaces the Sprint system. It is the execution mechanism *inside* a Sprint
...
Sprint-04-Step-02        (CURRENT_SPRINT.md: "Create repository-level CLAUDE.md")
 └─ Sprint-04-Step-02.1  (AI Bus: first execution attempt)
 └─ Sprint-04-Step-02.2  (AI Bus: re-run after a FIX, if needed)
```
