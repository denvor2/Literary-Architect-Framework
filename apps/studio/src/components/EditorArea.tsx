...
// Originally (Sprint 05/06) every mode called the exact same /api/line-editor
// endpoint — MODE_INFO existed purely as a perception layer, framing/labeling
// an identical response differently per role. That stopped being true as each
// role got its own real Expert (Critic — Sprint 08, Reader — Sprint 09,
// Co-author — Sprint 12); Editor is now the only mode still backed by
// /api/line-editor. MODE_INFO's per-role labels/framing below remain as UI
// copy regardless of which endpoint actually answers. See
// docs/product/EXPERT_CATALOG.md for the Visible Assistants this is meant to
// evoke.
//
...
function SceneImprove({
  ...
  chapterId?: string;
  bookTitle?: string;
  // Sprint-12-Step-04: the whole active Book, used as bookContext for
  // Co-author (always) and Editor (for consistency only — see handleImprove
  // below). Required, not optional — this component only renders when a
  // Book is already selected (see the `if (!book) return` branch above its
  // call site).
  book: Book;
  ...
  async function handleImprove() {
    setStatus("loading");
    try {
      // Sprint-12-Step-04: bookContext is now included alongside text — the
      // backend (/api/line-editor) uses it only for consistency (character
      // names, established facts), never to rewrite/expand beyond `text`;
      // this does not change what Editor does, only gives it context.
      // Routed through aiBus as an AIContextEnvelope (Sprint 06 Step 04);
      // the context values are carried alongside the operation but are not
      // read by aiBus yet — see aiBus.ts. Response is an AppliedAIResponse
  ...
  // Sprint-12-Step-04: Co-author is now genuinely connected (previously it
  // fictitiously called improve_text like every other non-Critic/Reader
  // mode — see the module-level "Perception layer only" comment near
  // AssistantMode, which described that old behavior). Uses the whole
  // scene's text (`text`, not getSelectedText()) — Co-author writes/
  // continues the scene as a whole, it does not react to a selected
  // fragment the way Critic/Reader do. Produces a Revision (new/continued
  // text), same as Editor, so it reuses the same generic preview branch
  // below (Original/Improved + "Заменить текст") — no new UI branch needed.
  async function handleCoauthor() {
    ...
    try {
      const result = await aiBus.execute({
        operation: {
          type: "coauthor_draft",
          payload: { currentText: text, bookContext: book },
        },
        context: {
          sceneId,
          chapterId,
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <button
        ...
        // Sprint-12-Step-04: Co-author must work on an empty scene (writing
        // a new draft from scratch, per /api/coauthor's own contract) — the
        // shared empty-text disable only applies to the other modes, which
        // all require existing text to act on.
        disabled={
          status === "loading" ||
