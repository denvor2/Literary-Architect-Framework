...
| Шаг | Описание | Файлы |
|-----|----------|-------|
| Step 01 | ADR-0012: серия книг — доменная модель (Series как контейнер для Book? Character на уровне Series?). | `docs/adr/ADR-0012-book-series.md` |
...
Sprint 22-25 — последовательная цепочка (каждый зависит от предыдущего).
...
6. **Sprint 26** (ЛитРес) — зависит от исследования API
7. **Sprint 27** (Серия) — требует ADR, низкая уверенность
